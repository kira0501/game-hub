...
Если нужно быстро открыть сайт без внешней базы, не добавляй `DB_HOST` и `DB_URL` в Vercel. В этом случае `api/index.php` включит демо-режим: создаст временную SQLite-базу в `/tmp` и выполнит миграции с сидами. После холодного старта данные могут пересоздаваться, поэтому для нормального показа диплома лучше подключить внешнюю MySQL.
