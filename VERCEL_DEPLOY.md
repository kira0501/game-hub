...
Если нужно быстро открыть сайт без внешней базы, не добавляй `DB_HOST` и `DB_URL` в Vercel. Во время build composer script `vercel` создаст демо SQLite-базу `database/vercel.sqlite`, а `api/index.php` при запуске скопирует её в `/tmp`. После холодного старта данные могут пересоздаваться, поэтому для нормального показа диплома лучше подключить внешнюю MySQL.
